...
#ifdef CONFIG_NIMBLE_PAGE_SELECTION
...
{
        gfp_t gfp_mask = GFP_USER | __GFP_DRAM;
        return alloc_page(gfp_mask);
}
#endif
                               ...
                               enum lru_list lru)
{
        ...
        // Return to active list if couldn't promote
...
static void scan_node(pg_data_t *pgdat, struct scan_control *sc)
{
		...
		for_each_evictable_lru(lru)
			...
			if (pmem_node_id !=-1 && sc->only_promote == 0 && (is_inactive_lru(lru)||is_active_lru(lru)))
				shrink_list(lru, nr_to_scan, lruvec, sc);

			if (sc->only_promote && (is_inactive_lru(lru) || is_active_lru(lru) || is_promote_lru(lru))) {
                    		shrink_list(lru, nr_to_scan, lruvec, sc);
			}
        	}
...
static int kpromoted(void *p)
{
	...
	if(pgdat->node_id == 0 || pgdat->node_id ==1)
		sc.only_promote = 0;
